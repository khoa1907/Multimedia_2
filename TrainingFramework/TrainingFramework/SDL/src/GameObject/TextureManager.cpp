...
void TextureManager::Render(int x, int y, int width, int height, Vector2 spritePos, Vector2 spriteSize, double angle, SDL_RendererFlip flip)
{
	SDL_Rect srcRect = { spritePos.x ,spritePos.y , spriteSize.x, spriteSize.y };
	SDL_Rect dstRect = { x - Camera::GetInstance()->GetPosition().x, y - Camera::GetInstance()->GetPosition().y, width , height };
